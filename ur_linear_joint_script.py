...
def linear_parabolic_blend(
    ...
    start_t = time.time()
    while True:
        if time.time() - start_t < blended_t:
            ut = init_pos + (0.5 * accel * ((time.time() - start_t) ** 2))
            # theta = inverse_kinematic(hdl_j, ut, theta)
            for count, hdl in enumerate(hdl_j):
                sim.setJointTargetPosition(hdl, ut[count])
            theta_bc = ut
        elif blended_t <= time.time() - start_t < end_time - blended_t:
            ut = (
                init_pos
                + (0.5 * accel * (blended_t**2))
                + (accel * blended_t * ((time.time() - start_t) - blended_t))
            )
            # theta = inverse_kinematic(hdl_j, ut, theta)
            for count, hdl in enumerate(hdl_j):
                sim.setJointTargetPosition(hdl, ut[count])
            theta_bc = ut
        elif end_time - blended_t <= time.time() - start_t < end_time:
            ut = (
                end_pos
                - (0.5 * accel * (end_time**2))
                + (accel * end_time * (time.time() - start_t))
                - (0.5 * accel * ((time.time() - start_t) ** 2))
            )
            # theta = inverse_kinematic(hdl_j, ut, theta)
            for count, hdl in enumerate(hdl_j):
                sim.setJointTargetPosition(hdl, ut[count])
...
def sysCall_thread():
    ...
    positions = np.random.uniform(low=0, high=np.pi * 0.6, size=(5, 6))
    print(positions)
